...
#if DEBUG>0
  SerialUSB.print("Par: ");
  SerialUSB.print(Kp_user);
  SerialUSB.print(" ");
  SerialUSB.print(Kd_user);
  SerialUSB.print(" ");
  SerialUSB.print(Kp_thr_user);
  SerialUSB.print(" ");
  SerialUSB.println(Ki_thr_user);
#endif

  // Calibration mode??
  if (OSCpush[2]==1)
  {
    SerialUSB.print("Calibration MODE ");
    angle_offset = angle_adjusted_filtered;
    SerialUSB.println(angle_offset);
  }
